    ...
    const fetchDashboardData = async () => {
      ...
      try {
        const [usersResponse, marksResponse] = await Promise.all([
          ...
          fetch(
            `${import.meta.env.VITE_BASE_URL}/external_interaction_marks/${externalId}`
          ),
        ]);

        if (!usersResponse.ok) throw new Error("Failed to fetch faculty list");
        if (!marksResponse.ok)
          throw new Error("Failed to fetch evaluation marks");
        ...
        const eligibleFaculty = allUsers.filter(
          (user) =>
            ...
            ALLOWED_DESG.includes(user.desg)
        );
        ...
        const formattedFacultyPromises = eligibleFaculty.map(
          async (faculty) => {
        ...
        const formattedFacultyList = await Promise.all(
          formattedFacultyPromises
        );
      ...
      <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
        <div className="bg-indigo-700 px-6 py-4">
          ...
          <p className="text-indigo-100 text-sm">
            Evaluate faculty members assigned for review.
          </p>
        </div>
