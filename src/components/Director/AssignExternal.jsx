...
const AssignExternal = () => {
    const [exetrnalList, setexetrnalList] = useState([]);
    ...
    useEffect(() => {
        const fetchFaculties = async () => {
            ...
            try {
                ...
                const adminFaculties = data.filter(
                    (faculty) =>
                        faculty.desg === "HOD" || faculty.desg === "Dean" &&
                        ALLOWED_ROLES.includes(faculty.role)
                );

                // Transform the data to match your component's structure
                const formattedFaculties = adminFaculties.map((faculty) => ({
                    id: faculty._id,
                    name: faculty.name,
    ...
    useEffect(() => {
        const fetchExternalFaculty = async () => {
            ...
            try {
                ...
                if (!response.ok) {
                    throw new Error("Failed to fetch external list");
                }
                ...
                setexetrnalList(formattedExternals);
            } catch (error) {
    ...
    useEffect(() => {
        const fetchAssignments = async () => {
            ...
            try {
                const response = await fetch(
                    `${import.meta.env.VITE_BASE_URL}/external-assignments`
                );
    ...
    useEffect(() => {
        // This will filter out assignments for externals that no longer exist in the exetrnalList
        if (exetrnalList.length > 0 && Object.keys(assignments).length > 0) {
            const validExternalIds = exetrnalList.map(external => external.id);
    ...
    const handleRemoveFaculty = async (externalId, internalFacultyId) => {
        try {
            ...
            // Make API call
            const response = await fetch(
                `${import.meta.env.VITE_BASE_URL}/assign-externals`,
                {
                    method: "POST",
                    headers: {
    ...
    const filteredInternalFaculty = internalFacultyList.filter(
        (faculty) =>
            ...
            (faculty.designation === "HOD" || faculty.designation === "Dean")
    );
            ...
            <div className="bg-white rounded-xl shadow-md overflow-hidden mb-8">
                ...
                <div className="bg-indigo-700 px-6 py-4">
                    ...
                    <p className="text-indigo-100 text-sm">
                        Manage faculty assignments for each external reviewer
                    </p>
                </div>

                <div className="p-6">
                    {loading && exetrnalList.length === 0 ? (
                                                    ...
                                                    {assignedFaculty && assignedFaculty.map((faculty) => {
                                                        ...
                                                        return (
                                                            ...
                                                            >
                                                                ...
                                                                {(
                                                                    // Check if isDirectorMarksGiven exists and is true
                                                                    faculty.isDirectorMarksGiven ? (
                                                                        <div className="flex items-center">
                                                                        ...
                                                                        <div>
                                                                            ...
                                                                            <button
                                                                                onClick={() => {
                                                                                    navigate(`/director-evaluate/${faculty._id}`, {
                                                                                        state: {
                                                                                            faculty: faculty,
                                                    ...
                                                    <th className="px-6 py-3 text-gray-600">
                                                        <div className="flex items-center">
                                                            <input
                                                                ...
                                                                onChange={() => {
                                                                    // Many-to-many: select/deselect all faculties
                                                                    const allSelected = filteredInternalFaculty.every(faculty =>
                                                                        pendingAssignments[selectedExternal.id]?.includes(faculty.id)
                                                                    );
                                                                    if (allSelected) {
                                                                        setPendingAssignments({
                                                                        ...
                                                                        setPendingAssignments({
                                                                            ...pendingAssignments,
                                                                            [selectedExternal.id]: [
                                                                                ...filteredInternalFaculty.map(faculty => faculty.id)
                                                                            ]
                                                                        });
                                                                    }
                                                                }}
