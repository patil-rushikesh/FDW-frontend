...
const DirectorInteractionEvaluation = () => {
  ...
  const handleSave = async (isSubmitted = false) => {
    ...
    try {
      ...
      if (isSubmitted) {
        const department = facultyDepartment|| "";
        const external_id = externalReviewer.id;

        const apiResponse = await fetch(`${import.meta.env.VITE_BASE_URL}/${department}/director_interaction_marks/${external_id}/${facultyId}`, {
          method: 'POST',
          headers: {
