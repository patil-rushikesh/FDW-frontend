...
const Research = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          // Patents
          patentCommercialized:
            data.patents.individualCommercialized.count || 0,
          ...
          internationalFellowship:
            data.awards.internationalFellowship.count || 0,
          nationalFellowship: data.awards.nationalFellowship.count || 0,

          // Grants and Revenue
          researchGrants: data.grantsAndRevenue.researchGrants.amount || 0,
          consultancyRevenue:
            data.grantsAndRevenue.consultancyRevenue.amount || 0,
          patentCommercialRevenue:
            data.grantsAndRevenue.patentCommercialRevenue.amount || 0,
          productCommercialRevenue:
            data.grantsAndRevenue.productCommercialRevenue.amount || 0,
          ...
          // Industry Internship/Placement
          industryInternshipPlacement:
            data.industryAssociation.internshipsAndPlacements.count || 0,
        });
        ...
        setProofLinks({
          ...
          patentCommercialized:
            data.patents.individualCommercialized.proof || "",
          ...
          internationalFellowship:
            data.awards.internationalFellowship.proof || "",
          nationalFellowship: data.awards.nationalFellowship.proof || "",
          researchGrants: data.grantsAndRevenue.researchGrants.proof || "",
          consultancyRevenue:
            data.grantsAndRevenue.consultancyRevenue.proof || "",
          patentCommercialRevenue:
            data.grantsAndRevenue.patentCommercialRevenue.proof || "",
          productCommercialRevenue:
            data.grantsAndRevenue.productCommercialRevenue.proof || "",
          ...
          industryInternshipPlacement:
            data.industryAssociation.internshipsAndPlacements.proof || "",
        });
      } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      driveFolderLink: formData.driveFolderLink, // Add this line
      papers: {
        sci: {
      ...
      bookChapters: {
        scopus: { count: Number(formData.scopusChapter) },
        other: { count: Number(formData.otherChapter) },
        marks: calculateBookChapterScore(),
      },
      books: {
        scopus: { count: Number(formData.scopusBooks) },
        national: { count: Number(formData.nationalBooks) },
        local: { count: Number(formData.localBooks) },
        marks: calculateBookScore(),
      },
      citations: {
        wos: { count: Number(formData.wosCitations) },
        scopus: { count: Number(formData.scopusCitations) },
        google: { count: Number(formData.googleCitations) },
        marks: calculateCitationScore(),
      },
      patents: {
        individualCommercialized: {
          count: Number(formData.patentCommercialized),
        },
        individualGranted: { count: Number(formData.patentGranted) },
        collegeCommercialized: { count: Number(formData.patentCollege) },
        collegeGranted: { count: Number(formData.patentCollegeGranted) },
        marks: calculatePatentScore(),
      },
      training: {
        revenue: { amount: Number(formData.trainingRevenue) },
        marks: calculateTrainingScore(),
      },
      nonResearchGrants: {
        amount: { value: Number(formData.nonResearchGrant) },
        marks: calculateNonResearchScore(),
      },
      products: {
        commercialized: { count: Number(formData.productCommercialized) },
        developed: { count: Number(formData.productDeveloped) },
        poc: { count: Number(formData.pocDeveloped) },
        marks: calculateProductScore(),
      },
      awards: {
        international: { count: Number(formData.internationalAward) },
        government: { count: Number(formData.governmentAward) },
        national: { count: Number(formData.nationalAward) },
        internationalFellowship: {
          count: Number(formData.internationalFellowship),
        },
        nationalFellowship: { count: Number(formData.nationalFellowship) },
        marks: calculateAwardScore(),
      },
      grantsAndRevenue: {
        researchGrants: { amount: Number(formData.researchGrants) },
        consultancyRevenue: { amount: Number(formData.consultancyRevenue) },
        patentCommercialRevenue: {
        ...
        startupRevenue: { amount: Number(formData.startupRevenue) },
        startupFunding: { amount: Number(formData.startupFunding) },
        marks: calculateGrantsAndRevenueScore(),
      },
      startupPCCOE: {
        revenue: { amount: Number(formData.startupRevenuePCCOE) },
        funding: { amount: Number(formData.startupFundingPCCOE) },
        products: { count: Number(formData.startupProductsPCCOE) },
        poc: { count: Number(formData.startupPOCPCCOE) },
        registered: { count: Number(formData.startupRegisteredPCCOE) },
        marks: calculateStartupPCCOEScore(),
      },
      industryInteraction: {
        activeMOU: { count: Number(formData.activeMOU) },
        labDevelopment: { count: Number(formData.labDevelopment) },
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-8 bg-gray-50 min-h-screen">
      {/* Google Drive Folder Link Section */}
      <SectionCard
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center">
              ...
              <div className="ml-3">
                <p className="text-sm text-yellow-700">
                  Please upload all your supporting documents to Google Drive
                  and share the folder link below. Make sure the folder is
          ...
          <div className="text-sm text-gray-600 mt-2">
            ...
            <ul className="list-disc ml-5 space-y-1">
              <li>
                Create a folder in Google Drive with all your supporting
                documents
              </li>
              <li>
                Name the folder as: {userData.name}_Research_Documents_
                {new Date().getFullYear()}
              </li>
              <li>Right-click the folder and select "Share"</li>
              <li>Change access to "Anyone with the link" and copy the link</li>
